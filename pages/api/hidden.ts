...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { initData, project_id, photo_url, stage, comment } = req.body || {};
  ...
  const { error } = await supabaseAdmin.from("hidden_works").insert({
    user_id: u.id, project_id: pId, photo_url, stage, comment
  });
